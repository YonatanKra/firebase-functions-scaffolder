...
function copyTemplateFile(packageName, suffix, functionsPath, spec) {
    ...
    // read main file
    let templateContent = fs.readFileSync(path.join(__dirname, './templates/') + `function.${suffix}.tmpl`, 'utf8');
    // replace %funcName% with packageName
    templateContent = replacePackageName(packageName, templateContent);
    // write file
    let writePath = `${functionsPath}/${packageName}/${packageName}.${suffix}`;
...
module.exports = {
    ...
    createPackage(functionsPath, packageName, typeScript = false) {
        functionsPath = functionsPath + (typeScript ? '/src' : '');
        this.createFolder(functionsPath, packageName);
